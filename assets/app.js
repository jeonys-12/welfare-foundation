...
async function boot(){try{const r=await fetch("data/news.json",{cache:"no-cache"});if(!r.ok)throw Error(r.status);const d=await r.json();all=d.items||[];$("#updatedAt").textContent=date(d.updated_at)+" "+new Date(d.updated_at).toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit"});const ai=d.ai_status||{};$("#aiStatus").textContent=ai.analyzed?("AI 분석 "+ai.analyzed+"건 · "+(ai.model||"OpenAI")):ai.reused?("AI 분석 재사용 "+ai.reused+"건"):"기본 요약 모드";tabs();lawTabs();periodTabs();render();renderLaw()}catch(e){$("#updatedAt").textContent="데이터 확인 필요";$("#newsGrid").innerHTML='<div class="empty"><b>데이터를 불러오지 못했습니다.</b><br>GitHub Actions를 실행해 주세요.</div>';$("#lawGrid").innerHTML='<div class="empty">법규 데이터를 불러오지 못했습니다.</div>';tabs();lawTabs();periodTabs()}}
